...
static uint8_t PPZ_BAT_VL_BYTE = 0x41; // "A" ( V BAT LEFT)
static uint8_t PPZ_BAT_IL_BYTE = 0x42; // "B" ( I BAT LEFT)
static uint8_t PPZ_BAT_VR_BYTE = 0x43; // "C" ( V BAT RIGHT)
static uint8_t PPZ_BAT_IR_BYTE = 0x45; // "E" ( I BAT RIGHT)
...
#define SONDA_DOWN 3
#define SONDA_UP 4
#define MEASURE_V_LEFT 5
#define MEASURE_I_LEFT 6
#define MEASURE_V_RIGHT 7
#define MEASURE_I_RIGHT 8
#define TELEMETRY_SN 9
...
uint8_t message_type = MEASURE_V_LEFT;
/*Quitar*/
int cont = 0;
/*-------------------*/
...
// Parse battery info. This could be done in only one function.
// Pass by argument the msg_id and choose the field updated (e.g., vbat_left)
// Depending on the msg_id
static void message_parse_v_left(void)
{
  uint8_t msgBytes[2] = {serial_msg.msgData[3], serial_msg.msgData[2]};
  serial_msg.time = serial_byteToint(msgBytes, 2);
  memset(msgBytes, 0, 2);
  msgBytes[0] = serial_msg.msgData[5];
  msgBytes[1] = serial_msg.msgData[4];
  serial_msg.vbat_left = serial_byteToint(msgBytes, 2);
}

static void message_parse_i_left(void)
{
  uint8_t msgBytes[2] = {serial_msg.msgData[3], serial_msg.msgData[2]};
  serial_msg.time = serial_byteToint(msgBytes, 2);
  memset(msgBytes, 0, 2);
  msgBytes[0] = serial_msg.msgData[5];
  msgBytes[1] = serial_msg.msgData[4];
  serial_msg.cbat_left = serial_byteToint(msgBytes, 2);
}

static void message_parse_v_right(void)
{
  uint8_t msgBytes[2] = {serial_msg.msgData[3], serial_msg.msgData[2]};
  serial_msg.time = serial_byteToint(msgBytes, 2);
  memset(msgBytes, 0, 2);
  msgBytes[0] = serial_msg.msgData[5];
  msgBytes[1] = serial_msg.msgData[4];
  serial_msg.vbat_right = serial_byteToint(msgBytes, 2);
}

static void message_parse_i_right(void)
{
  uint8_t msgBytes[2] = {serial_msg.msgData[3], serial_msg.msgData[2]};
  serial_msg.time = serial_byteToint(msgBytes, 2);
  memset(msgBytes, 0, 2);
  msgBytes[0] = serial_msg.msgData[5];
  msgBytes[1] = serial_msg.msgData[4];
  serial_msg.cbat_right = serial_byteToint(msgBytes, 2);
}
...
// Checksum
  ...
  switch (serial_msg.msg_id) {
    ...
    case SR_VL_BAT:
      message_parse_v_left();
      serial_msg.error_last = SERIAL_BR_ERR_NONE;
      message_type = MEASURE_V_LEFT;
      break;
    // Left Battery Current
    case SR_IL_BAT:
      message_parse_i_left();
      serial_msg.error_last = SERIAL_BR_ERR_NONE;
      message_type = MEASURE_I_LEFT;
      break;
    // Right Battery Voltage
    case SR_VR_BAT:
      message_parse_v_right();
      serial_msg.error_last = SERIAL_BR_ERR_NONE;
      message_type = MEASURE_V_RIGHT;
      break;
    // Right Battery current
    case SR_IR_BAT:
      message_parse_i_right();
      serial_msg.error_last = SERIAL_BR_ERR_NONE;
      message_type = MEASURE_I_RIGHT;
      break;
...
void serial_ping()
{
  ...
  */
  ...
  if (now_s > (last_s + SEND_INTERVAL)) {
    ...
    // Choose which type of message each time we need to send
    if (radio_control_get(RADIO_GAIN2) > 0) {
      message_type = SONDA_UP;
    } else if (radio_control_get(RADIO_GAIN2) < 0) {
      message_type = SONDA_DOWN;
    } else {
      message_counter = ((message_counter + 1) % 5);
      message_type = message_counter + 5;
    }
...
//    serial_read_message();
    switch (message_type) {
      ...
      case MEASURE_V_LEFT:
        ...
        serial_snd.msgData[0] = PPZ_START_BYTE;
        serial_snd.msgData[1] = PPZ_BAT_VL_BYTE;
        serial_snd.time = sys_time.nb_sec;
        message_type = MEASURE_V_LEFT;
        ito2h(serial_snd.time, msg_time);
        serial_snd.msgData[2] = msg_time[0];
      ...
      case MEASURE_I_LEFT:
        ...
        serial_snd.msgData[0] = PPZ_START_BYTE;
        serial_snd.msgData[1] = PPZ_BAT_IL_BYTE;
        serial_snd.time = sys_time.nb_sec;
        message_type = MEASURE_I_LEFT;
        ito2h(serial_snd.time, msg_time);
        serial_snd.msgData[2] = msg_time[0];
        ...
        break;
      // Right Battery Voltage
      case MEASURE_V_RIGHT:
        serial_snd.msg_length = 6;

        serial_snd.msgData[0] = PPZ_START_BYTE;
        serial_snd.msgData[1] = PPZ_BAT_VR_BYTE;
        serial_snd.time = sys_time.nb_sec;
        message_type = MEASURE_V_RIGHT;
        ito2h(serial_snd.time, msg_time);
        serial_snd.msgData[2] = msg_time[0];
        ...
        break;
      // Right Battery Current
      case MEASURE_I_RIGHT:
        serial_snd.msg_length = 6;

        serial_snd.msgData[0] = PPZ_START_BYTE;
        serial_snd.msgData[1] = PPZ_BAT_IR_BYTE;
        serial_snd.time = sys_time.nb_sec;
        message_type = MEASURE_I_RIGHT;
        ito2h(serial_snd.time, msg_time);
        serial_snd.msgData[2] = msg_time[0];
