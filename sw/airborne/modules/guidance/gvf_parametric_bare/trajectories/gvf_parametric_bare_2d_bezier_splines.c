...
{
  /* Select in which spline we are.
   * Check w: spline zero: 0 <= t <= 1, spline ones: 1 <= t <= 2;
   */
  ...
  if(n_seg < 0)
  {
    // w could be < 0 in that case go to first point of first segment
    n_seg = 0;
    tt = 0;
  }
  ...
  // Berstein polynomial and iterative variables
  int k; int p; int nu; int l;
  int n = order;
  float berst;

  // p = 0 curve. p = 1 first deriv. p = 2 second derivative and so on.
  for(p = 0; p < 3; p++)
  {
    for(k = 0; k <= n - p; k++)
    {
...
float berstein_poly(float t, int k, int n){
  return binom(k,n) * powf(t,k) * powf(1-t, n - k);
}
