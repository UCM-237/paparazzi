...
{
  kalman_filter.K2[3][0] = POS_FLOAT_OF_BFP(ins_int.ltp_pos.x);
  kalman_filter.K2[3][1] = POS_FLOAT_OF_BFP(ins_int.ltp_pos.y);
  kalman_filter.K2[3][2] = SPEED_FLOAT_OF_BFP(ins_int.ltp_speed.x);
  kalman_filter.K2[3][3] = SPEED_FLOAT_OF_BFP(ins_int.ltp_speed.y);
  kalman_filter.K2[3][4] = (*stateGetNedToBodyEulers_f()).psi;
  pprz_msg_send_INS_EKF(trans, dev, AC_ID,
                    kalman_filter.X, kalman_filter.K2[3], kalman_filter.K2[1], kalman_filter.K2[2], kalman_filter.K2[4]);
}
...
void ins_int_init(void)
{
  ...
  INT32_VECT3_ZERO(ins_int.ltp_accel);
    ...
    register_periodic_telemetry(DefaultPeriodic, PPRZ_MSG_ID_INS_Z, send_ins_z);  // Esta seguramente sobra
    register_periodic_telemetry(DefaultPeriodic, PPRZ_MSG_ID_INS_REF, send_ins_ref);
    register_periodic_telemetry(DefaultPeriodic, PPRZ_MSG_ID_INS_EKF, send_kf_status);
  #endif
...
void ins_int_propagate(struct Int32Vect3 *accel, float dt)
{
  ...
  // Como el barco tiene brujula, realmente no hace falta la velocidad angular
  float U[3] = {body_accel.x, body_accel.y, ang_vel->r};
  extended_kalman_filter_predict(&kalman_filter, U, dt);

  // Esto es para la telemetria
  kalman_filter.K2[1][0] = U[0];
  kalman_filter.K2[1][1] = U[1];
  kalman_filter.K2[1][2] = U[2];
...
void ins_int_update_gps(struct GpsState *gps_s)
{
  ...
  // Para la telemetria
  for (int i = 0; i < 5; i++) {
    kalman_filter.K2[2][i] = Y[i];
  }

  extended_kalman_filter_update(&kalman_filter, Y);
  // kalman_filter.X[2] = 100; // DEBUG
  ...
  if(enable_ekf_filter){
    ...
    // kalman_filter.X[4] is not needed in the boat (we have the compass)
    kalman_filter.X[4] = attitude.psi;

    // ESTO NO HACE FALTA, YA LO HACE EN ALGUN SITIO
    // struct NedCoor_f speed;
    // speed.x = kalman_filter.X[2];
    // speed.y = kalman_filter.X[3];
    // speed.z = 0.0f;
    // stateSetSpeedNed_f(MODULE_INS_EKF_ID, &speed);
  }
  else{
