...
if __name__ == '__main__':
	...
	if ids is not None and in_ports is not None and out_ports is not None:
		...
		for i in range(len(ids)):
			...
			pprz_proxy_args.append("--ac=" + str(ac_id) + ":" + str(out_port) + ":" + str(in_port))
	else:
	...
	try:
		...
		print(" ------------------------ ")
		print("Custom pprzlink_proxy arguments:\n\t" + ' '.join(pprz_proxy_args) + "\n")
		if verbose:
			pprz_proxy_args.append('-v')

		pprz_proxy_cmd = ['python3', PAPARAZZI_PROXY] + pprz_proxy_args
		process = subprocess.Popen(pprz_proxy_cmd)
		# Smoothly handle process termination
